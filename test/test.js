...
*
*/
var answerArrayConsumerID = "";
var answerArrayClinicID = "";
    ...
    it('Create new consumer/Successfull case + get ID', function (done) {
            api.post('/consumers')
                ...
                .end(function (err, res) {
                    ...
                    answerArrayConsumerID = res.body.res.id;
                    done();
                })
        });

    it('Patch Consumer object / change name param', function(done) {
            api.patch('/consumers/' + answerArrayConsumerID )
                .set('Accept', 'aplication/json')
                .send(
                    {
                        "name": "ddddddddddddddddddddddsssssssssssssssssssss",
                        "phone": "999999999999999999999999999999999999999999999999999999",
                        "receiveNotification": true,
                        "dontSentAdv": true,
                        "signedUp": true,
    ...
    it('Create new clinic/Successfull case + get ID', function (done) {
        api.post('/clinics')
            ...
            .end(function (err, res) {
                ...
                //expect(res.body).to.equal({});
                //expect(res.body.res.name).to.equal("namex");
                answerArrayClinicID = res.body.res.id;
                done();
            })
    });

    it('Patch clinic object / change name param', function(done) {
        api.patch('/clinics/' + answerArrayClinicID )
            .set('Accept', 'aplication/json')
            .send(
                {
                    name : clinic.name + '999999999999999999999999999999999999999999999999999999',
                    latitude : clinic.latitude,
                    longitude: clinic.longitude,
                    confirmed: clinic.confirmed
                })
